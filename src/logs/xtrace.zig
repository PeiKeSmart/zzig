...
pub fn PrintString(comptime fmt: []const u8, args: []u8) void {
    if (strings.Contains(fmt, "{s}")) {
        std.io.getStdOut().writer().print(fmt, .{args});
    } else {
        //@compileError("错误: 格式化字符串中缺少 '{s}' 占位符。");
        std.io.getStdOut().writer().print("Error: The '{s}' placeholder is missing from the formatted string.", .{"{s}"});
    }
}
...
pub fn PrintNumber(comptime fmt: []const u8, args: anytype) void {
    if (strings.Contains(fmt, "{d}")) {
        ...
        std.io.getStdOut().writer().print("Error: The '{s}' placeholder is missing from the formatted string.", .{"{d}"});
    }
}
